...
void keyball_oled_render_ballinfo(void) {
  ...
  // indicate scroll snap mode: "VT" (vertical), "HN" (horiozntal), and "SCR"
  // (free)
...
void keyball_handle_auto_mouse_layer_change(layer_state_t state) {
  ...
  // go into AML
  if (!layer_state_cmp(last_state, AUTO_MOUSE_DEFAULT_LAYER) &&
      layer_state_cmp(state, AUTO_MOUSE_DEFAULT_LAYER)) {
    keyball.total_mouse_movement = 0;
  } // go out AML
  else if (layer_state_cmp(last_state, AUTO_MOUSE_DEFAULT_LAYER) &&
           !layer_state_cmp(state, AUTO_MOUSE_DEFAULT_LAYER)) {
    set_auto_mouse_timeout(get_auto_mouse_keep_time());
    keyball.total_mouse_movement = 0;
  }
...
void keyboard_post_init_kb(void) {
  ...
  if (eeconfig_is_enabled()) {
    ...
    // set_auto_mouse_timeout(c.amlto == 0 ? AUTO_MOUSE_TIME : (c.amlto + 1) *
    // AML_TIMEOUT_QU);
